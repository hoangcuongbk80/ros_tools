...
Vector6f est_pose[21];
Matrix6f est_cov[21];
bool first_observe[21] = {true};
...
void loadModels()
{
  ...
  for(int i=0; i < model_paths.size(); i++)
  {
    ...
    {
      colorMap(2, color_model_cloud->points[k]);
    }
...
void extract_transform(std::string line, Eigen::Matrix4f &T, int class_index)
{
	  vector<string> st;
    boost::trim(line);
		boost::split(st, line, boost::is_any_of("\t\r "), boost::token_compress_on);
    T(0, 3) = std::stof(st[0]); T(1, 3) = std::stof(st[1]); T(2, 3) = std::stof(st[2]); //translaton
    T(0, 0) = std::stof(st[3]); T(0, 1) = std::stof(st[4]); T(0, 2) = std::stof(st[5]); // rotation 1st column
    T(1, 0) = std::stof(st[6]); T(1, 1) = std::stof(st[7]); T(1, 2) = std::stof(st[8]); // rotation 2nd column
    T(2, 0) = std::stof(st[9]); T(2, 1) = std::stof(st[10]); T(2, 2) = std::stof(st[11]); // rotation 3rd column
    float mean_dist = std::stof(st[12]);
    //std::cerr << T << "\n";

    Eigen::Affine3f pose_A;
    pose_A.matrix() = T;
    Vector3f rot = pose_A.rotation().eulerAngles(0, 1, 2);

    Vector6f meas_pose;
    meas_pose(0) = pose_A.translation()(0); meas_pose(1) = pose_A.translation()(1); meas_pose(2) = pose_A.translation()(2);
    meas_pose(3) = rot(0); meas_pose(4) = rot(1); meas_pose(5) = rot(3);
...
int main(int argc, char **argv)
{
  ...
  /* pcl::io::loadPLYFile<pcl::PointXYZ> (model_path, *model_cloud);
  float translation[3];
  float rot_quaternion[4];
  translation[0] = 0.058; translation[1] = -0.02; translation[2] = 0.617;
  Eigen::Vector3f b(translation[0], translation[1], translation[2]);
  rot_quaternion[0] = 0.4044174, rot_quaternion[1] = 0.7850157, rot_quaternion[2] = -0.3912824, rot_quaternion[3] = 0.259027;
  Eigen::Quaternionf a(rot_quaternion[0], rot_quaternion[1], rot_quaternion[2], rot_quaternion[3]);
  pcl::transformPointCloud(*model_cloud, *model_cloud, b, a);
  copyPointCloud(*model_cloud, *color_model_cloud);
  colorModel(2);
 */
    ...
    if (posefile.is_open())                     
    {
      if (!posefile.eof())                 
      {
        ...
        while(true)
        {
          ...
          extract_transform(line, T, cls_index);
